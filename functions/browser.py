...
from constants import user_agent, base_width, base_height
...
class Browser:
    ...
    async def browser_init(self, width: int = base_width, height: int = base_height, user_agent: str = user_agent):
        if not self.playwright and not self.browser:
            self.logger.info('Launching browser...')
            self.playwright = await async_api.async_playwright().start()
            self.browser = await self.playwright.firefox.launch(headless=not self.debug)
